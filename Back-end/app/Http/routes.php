...
$app->group(['prefix' => 'users', 'namespace' => 'App\Http\Controllers'], function() use ($app) {
	$app->post('/login', ['uses' => 'UsersController@loginUser', ]);
...
$app->group(['prefix' => 'posts', 'namespace' => 'App\Http\Controllers'], function() use ($app) {
	...
	$app->get('/home', ['uses' => 'PostsController@getHomePosts', ]);
	$app->get('{id}', ['uses' => 'PostsController@getPost', ]);
	$app->post('/', ['middleware' => 'auth','uses' => 'PostsController@createPost', ]);
	$app->delete('{id}', ['middleware' => 'auth','uses' => 'PostsController@deletePost', ]);
	$app->put('{id}', ['middleware' => 'auth' , 'uses' => 'PostsController@updatePost', ]);
});
